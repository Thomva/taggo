...
export default (label, placeholder, cssClass = '', inputID) => {
  ...
  return label !== '' ? new Handlebars.SafeString(`<div ${escapedCssClass !== '' ? `class="${escapedCssClass}"` : ''}><label for="${escapedInputID}" class="m-inputText__label">${escapedLabel}</label><input id="${escapedInputID}" type="email" placeholder="${escapedPlaceholder}" class="m-inputText__input"><span class="m-inputText__error"></span></div>`) : '';
};
